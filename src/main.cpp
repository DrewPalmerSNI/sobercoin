...
#if defined(NDEBUG)
# error "Sobercoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0f0b27509b9a2795d51575a68a05ecc3131a402396ca0e50965c334ada6a2912");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sobercoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Sobercoin Signed Message:\n";
...
{
    // Sobercoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Sobercoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Sobercoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Sobercoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Sobercoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sobercoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb794d6d037a2e221b744c6f7c99cb139c6e3013fa7e775c0a95a410404a2ea21");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Sobercoin is released!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 64 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045bc4513f0863d402071aed43eba44530f8462b68fc0804d2df29ff5136866ce5b12e0722157112c1e3319ddfc9a010e9bbc235703630736e4c6997fd7c3b89a7") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1516430466;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085408349;

        if (fTestNet)
        {
            block.nTime    = 1516430418;
            block.nNonce   = 386449828;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x3cb665f9fa29646c830d00b81f238071c53dc7182c599252a8ca874d1600a52c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc2, 0xbd, 0xd3 }; // Sobercoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SobercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SobercoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SobercoinMiner(CWallet *pwallet)
{
    printf("SobercoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sobercoin-miner");
    ...
    try { loop {
        ...
        printf("Running SobercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SobercoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SobercoinMiner, pwallet));
}
